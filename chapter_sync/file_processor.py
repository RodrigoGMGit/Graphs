...
def process_downloaded_files(
    downloaded_paths: Optional[list[Path]] = None,
    ...
    if downloaded_paths:
        files_to_process = [
            Path(p) for p in downloaded_paths if Path(p).suffix.lower() == ".xlsx"
        ]
    else:
    ...
    for file_path in files_to_process:
        try:
            ...
            logger.debug(f"Movido: {file_path} → {dest_path}")
            ...
            logger.error(
                f"Error inesperado al procesar {file_path}: {e}", exc_info=True
            )
            continue
...
def get_types_needing_download(files_dir: Path) -> dict[str, dict]:
    ...
    for file_type in file_types:
        ...
        if not type_dir.exists():
            ...
            logger.info(
                f"No se encontró directorio de archivos para {file_type}, se descargará"
            )
            needs_download = True
        else:
            ...
            if not xlsx_files:
                ...
                logger.info(
                    f"No se encontraron archivos para {file_type}, se descargará"
                )
                needs_download = True
            else:
...
def check_and_download_if_needed(files_dir: Path) -> None:
    ...
    try:
        ...
        types_needing_download = [
            file_type
            for file_type, info in types_info.items()
        ...
        # Re-load .env with logging when we have a logger available
        ...
        # Re-load .env silently (no need to log search paths in GUI)
        _load_env_with_logging(logger=logger, silent=True)
        ...
        for file_type in types_needing_download:
            ...
            if local_date is None:
                # No local file - first run, proceed directly with download
                # No need to check remote date, just download
                logger.info(
                    f"{file_type}: No hay archivo local, procediendo con descarga"
                )
                types_to_actually_download.append(file_type)
            else:
                # Have local file - check remote date to compare
